...
import { createClient } from "@libsql/client"; // ✅ Nueva importación para Turso
...
app.delete("/api/evaluaciones", async (req, res) => {
    try {
        // Ejecutar consulta de conteo con db.execute()
        const countResult = await db.execute("SELECT COUNT(*) AS total FROM evaluaciones");
        // El resultado de libsql está en el array 'rows'
        ...
        console.error("Error al borrar todos los registros en Turso:", err);
...
app.post("/api/evaluaciones", async (req, res) => {
    try {
        ...
        // Ejecutar inserción con db.execute()
        await db.execute({
            sql: `INSERT INTO evaluaciones (app_name, descripcion, resultado, scores_json, pesos_json, comentario, fecha)
            VALUES (?, ?, ?, ?, ?, ?, ?)`,
            args: [app_name, descripcion, resultado, scores_json, pesos_json, comentario, fecha]
        });

        res.json({ ok: true });
    } catch (err) {
        console.error("Error al guardar la evaluación en Turso:", err);
...
app.get("/api/evaluaciones", async (req, res) => {
    try {
        // Ejecutar consulta de selección con db.execute()
        const result = await db.execute("SELECT * FROM evaluaciones ORDER BY fecha DESC");
        // Devolver solo las filas
        res.json(result.rows); 
    } catch (err) {
        console.error("Error al cargar las evaluaciones desde Turso:", err);
        res.status(500).json({ error: "Error al cargar las evaluaciones" });
    }
});

// API Key de Google PageSpeed
// Usar variable de entorno para la clave de API (MEJOR PRÁCTICA)
const GOOGLE_API_KEY = process.env.GOOGLE_API_KEY || "AIzaSyBI745APBmHfTfHuc1-9XqzjqtkyQvS--0";
...
// 🚀 Iniciar servidor (CORRECCIÓN para Railway)
const PORT = process.env.PORT || 3000;

app.listen(PORT, '0.0.0.0', () => { // Escucha en 0.0.0.0 y usa el puerto de Railway
